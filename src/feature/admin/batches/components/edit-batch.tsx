...
export function EditBatchForm({ params }: { params: { id: string } }) {
  ...
  const form = useForm<BatchFormData>({
    ...
    defaultValues: {
      ...
      startDate: batch ? new Date(batch.startDate) : undefined,
      endDate: batch ? new Date(batch.endDate) : undefined,
      status:
        (batch?.status as "Upcoming" | "Ongoing" | "Completed") || "Upcoming",
      instructors: batch?.instructors || [],
      maxLearners: batch?.maxLearners || 25,
  ...
  const onSubmit = async (values: BatchFormData) => {
    try {
      console.log("Edited batch data:", values);
    ...
    <div className="container mx-auto p-6 max-w-2xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the details for this batch.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
